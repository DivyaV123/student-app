...
const filteredAudio = computed(() => {
    return webrtc.audioTracks
})

const deletedAudio = computed(() => {
  webrtc.deletedAudioTracks.forEach((value, key) => {
...
const gridCols = computed(() => {
  let vidSize = webrtc.videoTracks.size
  if (vidSize > 4) {
    return 4
  }
  return vidSize
...
const joinRoom = async () => {
  try {
    ...
    if (joined.value == false) {
      ...
      roomStore.add_student_attendance({role:"student",batch:roomBatch.value.id})
...
const socketConnected = computed(() => {
  if(!webrtc.socketConnected){
    loadingMessage.value =
      'Trying to reconnect to server, this may take a few seconds, please wait or check your internet connection'
  }
...
onBeforeMount(async () => {
  ...
  theStore.setPageTitle("Classroom");
  await roomStore.get_single_batch(room.value);
  await webrtc.getWebRtc()
  await webrtc.connectSocket('https://v3streaming.qspiders.com')
  await webrtc.checkRoom(room.value)
  }
  else{
    if(route.params&&route.params.id){
      router.push({ name: "login", query: route.params});
    }
    else{
      router.push("/login");
    }
...
<template>
  <section class="pt-4">
    <div
      v-if="pageLoading || !socketConnected"
      wire:loading
    ...
    <div class="">
      <div class="col-span-6">
        <div class="">
          <div class="text-gray-500 dark:text-gray-400">
            <div
              class="flex flex-wrap md:justify-between border-b border-gray-400 mb-1"
            >
              <div>
               
                <div class="gap-1 flex flex-wrap">
                  <h6 class="text-orange-500 text-lg font-semibold">{{ room }}</h6>
                </div>
              </div>

              <div  class="flex">
                <div v-if="roomBatch && roomBatch.coursedetails" class="text-blue-500 capitalize font-semibold p-1">
                  {{ roomBatch.coursedetails.subject.displayname }}
                </div>
                <!-- <div class="text-blue-500">
                  <icon :path="mdiCogBox" />
                </div>
                <div v-if="joined" class="pr-8 text-green-500">
                  <span class="relative inline-block">
                    <icon :path="mdiPoll" />
                    <span
                      class="absolute top-0 right-0 inline-flex items-center justify-center text-xs font-bold leading-none transform translate-x-full -translate-y-1/2 text-gray-300"
                      >10/15</span
                    >
                  </span>
                </div> -->
            ...
            <div class="" v-if="joined">
              <div id="filtered-audio" class="grid gap-2 grid-cols-1 md:grid-cols-6 h-stream-container"> 
              <div
              id="videos"
              :class="audioSize > 0 ? 'md:col-span-5' : 'md:col-span-6'"
            >
            <div class="grid grid-cols-1 gap-2" :class="gridCols > 1 ? 'md:grid-cols-2': 'md:grid-cols-1'">
              ...
              <div class="flex flex-col  items-center w-full max-w-xs p-4 bg-gray-600 rounded-3xl md:flex-row">
                  <div :class="isTrainer?'border-green-500':'border-red-500'" class="-mt-28 md:-my-16 md:-ml-32 rounded-full border bg-gray-600" >
                  <img
                      class="w-auto h-36"
                      src="../assets/default_trainer.png"
                      alt="Trainer"
                  />
                  </div>
                  ...
                  <div class="flex flex-col items-center md:items-start">
                      ...
                      <p class="text-base font-medium text-red-500">Screen is not shared</p>
                  </div>
                  </div>
              </div>
          </div>

            <CardBox  v-for="[key, video] in videoTracks" :key="key"  :title="video.name">
              <div class="flex justify-center object-scale-down">
                <video :id="key" class="object-scale-down" muted :srcObject="video.stream" autoplay></video>
                <div class="hidden">
                  {{ handleFS(key) }}
                </div>
                <i class="watermark">&#169; QSpiders</i>
              </div>
            </CardBox>
          </div>
          </div>
          <div  class="col-span-1 md:block hidden">
            <div class="flex flex-col overflow-y-auto">
                  <CardBox
                  v-for="[key, user] in webrtc.audioTracks"
                  class="w-auto h-auto"
                    :title="user.name"
                    :key="key"
                  >
              ...
              <div class="flex w-full justify-center absolute bottom-2">
                ...
                <div class="block ml-1">
                  ...
                  <span class="px-1">
                    <BaseButton
                      :class="animateChat?'animate-pulse' : ''"
                      rounded-full
      ...
      >
        ...
        >
          ...
          <DividerHorizontal />
          <ButtonTextLink
            :class="checkRTC.hasMicrophone ? 'text-gray-400' : 'text-red-600'"
            :label="
          ...
          <DividerHorizontal />
          <ButtonTextLink
            :class="checkRTC.hasWebcam ? 'text-gray-400' : 'text-red-600'"
            :label="
        ...
        <template #footer>
          <BaseLevel mobile>
            <BaseButtons>
              <BaseButton
              v-if="roomBatch.status === 1 || roomBatch.status === 3"
                color="info"
      ...
      <div
        v-if="chatValue"
        class="fixed bg-slate-500 bottom-[62px] right-2 md:right-4  lg:w-1/3 opacity-95 sm:w-1/2 md:w-2/5 w-4/5"
      >
        <div class="w-full rounded">
          <CardBox class="h-chat-container md:rounded-none">
            <div class="flex justify-between border-b border-gray-500 flex-wrap"
            >
              <div class="flex gap-2">
                <div>
                    <input type="text" v-model="searchChatUser" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-5 w-40" placeholder="Search">
                </div>
                <div>
                 
                 <icon @click="changeFilterType('chat')"  :class="filterType == 'chat'?'text-green-500':''" :path="mdiChat" class="w-4 h-4 cursor-pointer" />
               </div>
              </div>
              ...
              <div class="flex gap-3 flex-wrap">
                ...
                <div class="ml-4" @click="clickChat">
                  <icon
                    :path="mdiCloseBox"
                    class="w-5 h-5 cursor-pointer hover:text-gray-300"
                  />
                </div>
              </div>
            </div>
            ...
            >
              <div
                v-for="[key, client] in filterdUsers" :key="key" 
                class="flex justify-between flex-wrap border-b border-gray-500 items-center pt-1"
              >
                <div class="flex gap-2">
                  <!-- <div>
                    <icon
                    :class="client.raised?'text-blue-500':'text-gray-500'"
                      :path="mdiHandBackLeft"
                      class="w-4 h-4 cursor-pointer"
                    />
                  </div> -->
                  <div @click="openChat(key)">
                    <h6 :class="client.connected ? 'text-green-500' :'text-red-500'" class="text-sm font-semibold cursor-pointer">
                      {{ client.name }} <span class="italic text-xs text-orange-500 capitalize">{{ client.role }}</span>
                    </h6>
                  </div>
                </div>
      ...
      <div class="flex gap-3 fixed bottom-[62px] right-7">
      <div
        class=" bg-slate-900"
        :class="showBottomChat?'':'hidden'"
      >
        <div class="w-80 rounded shadow-2xl">
          ...
          >
            <div class="flex justify-center items-center">
              <span v-if="activeChatUser && activeChatUser.value" :class="activeChatUser.value.connected ? 'text-green-500' :'text-red-500'"  class="font-semibold ml-1">{{
          ...
          <div
            class="overflow-auto px-1 py-1 flex flex-col-reverse"
            style="height: 19rem"
          >
           <div v-if="activeChatUser && activeChatUser.value">
            <div
              class=""
              v-for="(message, index) in activeChatUser.value.messages"
              :key="index"
            >
              <div
                v-if="message.from == mainStore.user.uid"
                class="flex justify-end pt-2 pl-10"
              >
                <span
                  class="bg-blue-900 h-auto text-gray-200 text-xs font-normal rounded-sm p-1 items-end justify-end"
                  >{{ message.content }}
              ...
              <div v-if="message.to == mainStore.user.uid || message.to ==='all'" class="flex items-center pt-2 pr-10">
                <span
                  class="ml-1 h-auto bg-green-900 text-gray-200 text-xs font-normal rounded-sm px-1 p-1 items-end"
                >
                  {{ message.content }}
                  <span class="text-gray-400 italic pl-1 text-xs">{{
                    message.date
          ...
          <form @submit.prevent="onSubmit">
            <div class="flex items-center p-1">
              ...
              <div
                class="w-7 h-7 rounded-full pl-4 bg-blue-300 text-center items-center flex justify-center"
              >
                <BaseButton
                  type="submit"
                  color="info"
...
<!-- Second popup g-talk -->
    ...
    <CardBoxModal
      :modelValue="startBroadcast"
      title="Start broadcast"
      button-label="Start"
      button="success"
      has-cancel
      @confirm="clickBroadcast"
      @cancel="startBroadcasting"
    >
