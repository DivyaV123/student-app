...
onBeforeMount(async () => {
  theme.setPageTitle("Closed Batches List")
  ...
  if (user.$state.loggedIn === false) {
    ...
    if (user.validation_status === 200) {
      ...
      //   router.push("/profile")
      // }
      await user.getstudentbatches(true,0)
    }
    else {
      await user.logout()
      router.go()
...
const searchParams = async() => {
  ...
  pageLoading.value = true
   ...
   await  user.getstudentbatches(true,0,params);
    pageLoading.value = false
...
const pageChange = async (page) => {
    ...
    currentPage.value = page+1;
    if (params) {
    await user.getstudentbatches(true,page,params);
    }
    else{
      await user.getstudentbatches(true,page);
    }
    // }
    pageLoading.value = false
    window.scroll({
  ...
  <div v-if="subject_batch" class="grid xl:grid-cols-5 sm:grid-cols-2 gap-4 md:grid-cols-2 lg:grid-cols-3">
    ...
    <div v-for="client in subject_batch" :key="client.id" class="card shadow-lg bg-gray-900  p-4 rounded-md mb-0">
      ...
      <h5 v-else class="text-sm border-b border-gray-600 pb-1 mb-1 text-gray-300"><b>Started at : </b>{{
        ...
        <!-- <h5 v-if="client.started_at && client.started_at.charAt(0) != '0'"
        class="text-sm border-b border-gray-600 pb-1 mb-1 text-gray-300"><b>Class Timings : </b>{{
          dayjs(client.started_at).format("hh:mm A") }}</h5> -->
      <h5
        class="text-sm border-b border-gray-600 pb-1 mb-1 text-gray-300"><b>Class Timings : </b>{{
      ...
      <div class="flex flex-wrap pb-1   mt-3">
        <!-- <jb-button
          v-if="client.mode_of_class && client.mode_of_class.name.toLowerCase() === 'online' && client.blocked === false && dayscrossed === true && profilled===false"
          label="Add Placement Form" type="submit" @click ="add_profile" color="success"
          class="font-medium text-sm py-1" />
        <jb-button
          v-if="client.mode_of_class && client.mode_of_class.name.toLowerCase() === 'online' && client.blocked === false && ((dayscrossed === false) || (dayscrossed === true&&profilled===true)) "
          label="Join classroom" type="submit" :href='"/classroom/" + client.batchplan.name' color="success"
          class="font-medium text-sm py-1" /> -->
          <!-- <jb-button
        ...
        <!-- <jb-button
          ...
          label="Classes" type="submit" href="https://qspiders.com/v4-online-classes" target="_blank" color="info"
          class="font-medium text-sm py-1" />
        ...
        <jb-button v-if="client.mode_of_class && client.mode_of_class.name.toLowerCase() === 'offline'"
          ...
          <!-- <jb-button
          label="Closed" type="submit" color="danger" class="font-medium text-sm py-1" />  -->
          <jb-button 
