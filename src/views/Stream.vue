...
onBeforeMount(async () => {
  ...
  if (mainStore.loggedIn) {
    await mainStore.getvalidations();
    if(isTauri){
    webrtc.isTauri = true
  ...
  await roomStore.get_batch_status(room.value);
  let server = "https://v4streaming7.qspiders.com"
...
<template>
  ...
  <div
    ...
    <div
      ...
      <span class="text-orange-500 text-lg font-semibold">{{ room }} 
      </span>
      <div  class="flex p-1 md:p-0">
                <div v-if="roomBatch && roomBatch.subject" class="text-blue-500  font-semibold uppercase">
                  {{ roomBatch.subject.displayname }}
                </div>
                </div>
    </div>
    <div class="p-2">
    ...
    <div
      ...
      <BaseLevel mobile>
            <BaseButtons>
              ...
              />
              <BaseButton
              v-else-if="roomBatch.dayscrossed === true && mainStore.validation.other===false"
                color="danger"
                :icon="mdiCastConnected"
                small
                label="Please fill placement form"
                to="/profile"
              />
              <BaseButton
              v-else-if="roomBatch.dayscrossed === true && mainStore.validation.other===true && mainStore.validation.image===false"
                color="danger"
                :icon="mdiCastConnected"
                small
                label="Add Proper Image"
                to="/view_profile"
              />
              
              <BaseButton
              v-else-if="roomBatch.status === 1 || roomBatch.status === 3"
                color="info"
                :icon="mdiCastConnected"
                small
                label="Join class"
...
<BaseButton label="Audio" v-if="audioValue" rounded-full color="success" :icon="mdiMicrophone" small
  @click="clickAudio" />
  <BaseButton label="Audio" v-else-if="audioRequest" rounded-full color="info" :icon="mdiMicrophone" small
  @click="clickAudio" />
  <BaseButton label="Audio" v-else rounded-full color="danger" :icon="mdiMicrophoneOff" small
  @click="clickAudio" />
</span>
<span class="px-1">
<BaseButton label="Chat" v-if="chatValue" :class="animateChat ? 'animate-pulse' : ''" rounded-full
  color="success" :icon="mdiMessageText" small @click="clickChat" />
  <BaseButton label="Chat"  v-else :class="animateChat ? 'animate-pulse' : ''" rounded-full
  color="danger" :icon="mdiForumRemove" small @click="clickChat" />
</span>
<span class="px-1">
<BaseButton label="Exit" rounded-full :color="exitValue ? 'info' : 'info'" :icon="mdiExitToApp" small
  @click="clickExit" />
</span>
<span class="px-1">
  <BaseButton
                href="https://qspiders.com/classes"
                rounded-full
            ...
            <div
        v-if="chatValue"
        class="fixed bg-slate-500 bottom-[62px] right-2 md:right-2 opacity-95 md:w-[20rem]"
      >
        <div class="w-full rounded">
          <CardBox class="h-mobile-main md:h-chat-container md:rounded-none">
            ...
            >
              <div class="flex gap-2">
                ...
                <div>
                 ...
                 <icon @click="changeFilterType('chat')"  :class="filterType == 'chat'?'text-green-500':''" :path="mdiChat" w="4"
                  h="4"  class="cursor-pointer" />
               </div>
              </div>
            ...
            class="h-mobile-inside md:h-student-container  overflow-y-auto"
            >
              <div
                v-for="[key, client] in filterdUsers" :key="key" 
                class="border-b border-gray-500 pt-1"
              >
          ...
          <nav
            ...
            <div class="flex justify-center items-center">
              <span v-if="activeChatUser && activeChatUser.value"
                class="font-semibold ml-1 text-gray-200">{{
          ...
          <div class="overflow-auto break-all px-1 py-1 flex flex-col-reverse" style="height: 19rem">
            <div v-if="activeChatUser && activeChatUser.value">
              <div class="break-all" v-for="(message, index) in activeChatUser.value.messages" :key="index">
                ...
                <div v-if="message.to == mainStore.user.uid || message.to === 'all'" class="flex items-center pt-2 pr-10 break-all">
                  <span class="break-all ml-1 h-auto bg-green-900 text-gray-200 text-xs font-normal rounded-sm px-1 p-1 items-end">
                    {{ message.content }}
                    <span class="text-gray-400 italic pl-1 text-xs">{{
                      message.date
                    }}</span>
