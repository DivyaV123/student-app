...
const send = async ()  => {
  ...
  if(!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(form.mail)))
  {
    ...
    var countt = JSON.parse(localStorage.getItem("email_count"))
    if (countt)
    {
      if(countt.date===dayjs(new Date).format("YYYY-MM-DD")&&countt.email===form.mail.trim()){
      ...
      couunt++;
      counts.value = couunt
      user.error_send_mail=[]
 
      if(counts.value<=5){
        localStorage.setItem("email_count", JSON.stringify({countz:counts.value,email:form.mail.trim(),date:dayjs(new Date).format("YYYY-MM-DD")}));
        await user.forgotsendemail({"email":form.mail})
   
        if(user.send_mail.message==="The otp has been sent to your mail id , please do check spam if you dont find it in inbox"){
        theme.pushMessage(user.send_mail.message) 
          pageLoading.value = false
          isverify.value=true 
        }
        else{
              theme.pushMessage(user.error_send_mail.message)
              pageLoading.value = false 
        }
      }
      else{
...
const verifyotp = async ()  => {
  ...
  {
  try{  
  await user.verifyotp({"otp" : JSON.parse(data1),"email" : form.mail})
